...
const normalizeAddressItem = (value: any, fallbackType: CustomerAddressType = 'other'): CustomerAddress | null => {
  ...
  if (typeof value === 'object') {
    ...
    return {
      ...
      registeredName: value.registeredName ? String(value.registeredName).trim() : undefined,
      registrationNo: value.registrationNo ? String(value.registrationNo).trim() : undefined,
      taxNo: value.taxNo ? String(value.taxNo).trim() : undefined,
      countryCode: value.countryCode ? String(value.countryCode).trim().toUpperCase() : undefined,
      city: value.city ? String(value.city).trim() : undefined,
      region: value.region ? String(value.region).trim() : undefined,
      postalCode: value.postalCode ? String(value.postalCode).trim() : undefined,
    };
  }
...
const splitAddressText = (value: string) => value.split(ADDRESS_TEXT_SEPARATOR);
...
export const normalizeCustomerAddresses = (source?: CustomerAddressSource | CustomerAddress[] | string | null): CustomerAddress[] => {
  ...
  if (Array.isArray(raw)) {
    ...
    try {
      ...
      items = Array.isArray(parsed) ? parsed : splitAddressText(trimmed);
    } catch {
      items = splitAddressText(trimmed);
    }
  }
...
export const getCustomerAddressSummary = (address: CustomerAddress | null | undefined) => {
  if (!address) return '';
  const label = address.label ? `${address.label}: ` : '';
  return `${label}${address.fullAddress}`;
};
