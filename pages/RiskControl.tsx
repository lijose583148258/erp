  ...
  const columns: Column<Invoice>[] = [
        ...
        return (
          <div className="flex flex-col min-w-[140px]">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-xs text-slate-400 font-bold">/ {formatPrice(row.amount)}</span>
            </div>
    ...
    },
    {
      header: t.riskStatus, key: 'days', accessor: (row) => {
        ...
        return (
          <span className={`px-4 py-1.5 rounded-2xl text-xs font-black uppercase tracking-widest border transition-all ${ui.color}`}>
            {ui.label}
          </span>
        );
      }
    },
    {
      header: t.history, key: 'freq', accessor: (row) => (
        <button
          ...
          className="flex items-center text-xs font-black text-blue-600 bg-white dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 transition-all shadow-sm active:scale-95"
        >
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-slate-900 dark:bg-black h-14 flex items-center overflow-hidden rounded-[24px] shadow-2xl border border-slate-800">
        ...
        <div className="flex-1 overflow-hidden">
          <div className="flex whitespace-nowrap animate-[ticker_60s_linear_infinite] hover:[animation-play-state:paused] items-center h-full">
            {stats.tickerItems.length > 0 ? (
              ...
              <span className="text-emerald-400 font-black text-xs mx-16 uppercase tracking-[0.2em]">{t.tickerNoOverdue}</span>
            )}
          </div>
        </div>
        <div className="hidden sm:flex bg-slate-800 dark:bg-slate-900 h-full px-4 md:px-8 items-center z-10 border-l border-slate-700">
          <span className="text-white/40 text-xs font-black uppercase whitespace-nowrap mr-3">{t.totalOverdue}</span>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 font-bold text-xs uppercase tracking-[0.3em] mt-3 opacity-60">{t.crmSub}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-10">
              ...
              <div className="text-right">
                <p className="text-xs font-black text-rose-400 uppercase tracking-widest mb-1">{t.globalExposure}</p>
                <span className="px-3 py-1 bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 text-xs font-black rounded-full border border-rose-100 dark:border-rose-800">{t.critical}</span>
              </div>
            </div>
            ...
            <div className="flex items-baseline space-x-2">
              ...
              <span className="text-xs font-bold text-rose-500 bg-rose-50 dark:bg-rose-900/20 px-2 py-0.5 rounded">+12.5%</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 dark:bg-slate-950 p-10 rounded-[40px] shadow-2xl relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-xs font-black text-white/30 uppercase tracking-[0.2em] mb-3">{t.longestDelay}</p>
            ...
            <div className="mt-6 flex items-center p-4 bg-white/5 rounded-2xl border border-white/5">
              ...
              <p className="text-xs text-slate-300 font-bold uppercase tracking-widest truncate">{stats.maxDaysCustomer}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group hover:shadow-2xl transition-all duration-500">
          ...
          <div className="relative z-10">
            ...
            <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3">{t.expectedToday}</p>
            ...
            <div className="mt-8 flex gap-2 items-center">
              ...
              <span className="text-xs font-black text-emerald-600 uppercase tracking-widest">{`${summary?.dueSoonCount || 0} ${t.activeSettlement}`}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-slate-400 font-bold mt-2 uppercase tracking-widest">{t.blindspotSubtitle}</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {blindSpots.map((spot, idx) => (
            <div key={idx} className="bg-slate-50 dark:bg-slate-800/60 rounded-2xl p-4 border border-slate-100 dark:border-slate-800">
              ...
              <p className="text-xs text-slate-400 font-bold mt-3 uppercase">{spot.action}</p>
            </div>
          ))}
        </div>
      </div>
