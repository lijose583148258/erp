...
export function CRMViewSwitcher({
 ...
 <button onClick={() => onViewChange('my')} className={`min-h-11 rounded-lg px-5 py-2.5 text-xs font-black transition-colors duration-150 motion-reduce:transition-none ${viewMode === 'my' ? 'bg-slate-900 text-white dark:bg-slate-700' : 'text-slate-600 dark:text-slate-300'}`}>
            {t.myCustomers}
          </button>
 <button onClick={() => onViewChange('public')} className={`min-h-11 rounded-lg px-5 py-2.5 text-xs font-black transition-colors duration-150 motion-reduce:transition-none ${viewMode === 'public' ? 'bg-rose-600 text-white' : 'text-slate-600 dark:text-slate-300'}`}>
        ...
        {canCreate && (
 <button data-testid="crm-add-customer" onClick={onCreate} className="flex min-h-11 items-center rounded-xl bg-blue-600 px-5 py-2.5 text-xs font-black text-white shadow-sm transition-colors duration-150 hover:bg-blue-700 motion-reduce:transition-none">
 ...
 className={`min-h-10 rounded-lg px-4 py-2 text-xs font-bold transition-colors duration-150 motion-reduce:transition-none ${
 ...
 aria-label={t.search || '搜索客户'}
            value={searchKeyword}
            onChange={(event) => onSearchKeywordChange(event.target.value)}
            maxLength={120}
            placeholder={t.search || "Search..."}
 className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm font-bold text-slate-700 outline-none transition-[border-color,box-shadow] duration-150 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 motion-reduce:transition-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
          />
        </div>
      </div>
    </div>
