        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {stats.map((stat) => (
                    <div key={stat.label} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between dark:bg-slate-900 dark:border-slate-800">
                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-4">{stat.label}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {laneStats.map((lane) => (
                    <div key={lane.route} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm dark:bg-slate-900 dark:border-slate-800">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-2">{t.route || '运输线路'}</p>
                                <p className="text-lg font-black text-slate-800 dark:text-white">{lane.route}</p>
                            </div>
                            <MapPin size={18} className="text-blue-500" />
                        </div>
                        <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                            <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl py-3"><p className="text-[10px] text-slate-400 font-black uppercase">{t.records || '记录'}</p><p className="text-lg font-black text-slate-800 dark:text-white">{lane.total}</p></div>
                            <div className="bg-cyan-50 dark:bg-cyan-900/20 rounded-2xl py-3"><p className="text-[10px] text-cyan-600 font-black uppercase">{t.coldChain || '冷链'}</p><p className="text-lg font-black text-cyan-700 dark:text-cyan-300">{lane.coldChain}</p></div>
                            <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl py-3"><p className="text-[10px] text-slate-400 font-black uppercase">{t.temperature || '温度'}</p><p className="text-lg font-black text-slate-800 dark:text-white">{lane.avgTemp}°C</p></div>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <button type="button" onClick={onGenerateAiInsights} className="px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-amber-200 text-amber-700 bg-amber-50 hover:bg-amber-100">AI预警面板</button>
                </div>
                    ...
                    {coldChainSeries.map(({ shipment, series, max, min }) => {
                        ...
                        return (
                            <div key={shipment.id} className="border border-slate-100 dark:border-slate-800 rounded-2xl p-4 bg-slate-50/60 dark:bg-slate-900/40">
                                ...
                                <div className="flex items-center justify-between">{renderSparkline(series)}<div className="text-right"><p className="text-2xl font-black text-cyan-600">{latest}°C</p><p className="text-[10px] text-slate-400 font-bold uppercase">{t.temperatureRange || '范围'}: {min}~{max}°C</p></div></div>
                            </div>
                        );
                    })}
                </div>
