...
export function ProductionBomSection({
  ...
  return (
    <section className="xl:col-span-5 space-y-5">
      <details className="rounded-[24px] border border-blue-100 bg-blue-50/60 px-4 py-3 dark:border-blue-900/40 dark:bg-blue-950/20">
        <summary className="cursor-pointer text-sm font-black text-blue-800 dark:text-blue-200">
          第一次填写？展开查看“配方主档 → 原料明细 → 保存回读 → 工单执行”
        </summary>
        <div className="mt-4">
          <DocumentInputGuide
            ...
            description="这里不是普通物料列表。对胶水、树脂、涂料来说，BOM 本质是配方版本：先说明做什么产品、哪个版本、按什么批量生产，再用表格录入原料、代号、占比、单耗、损耗和工序。"
            tone="blue"
            steps={[
              { title: '配方主档', description: '填写产品名称、版本、配方类型、输出单位、标准批量。', badge: '主数据' },
              { title: '原料明细表', description: '每一横行只代表一种原料；实际投料不在这里填写。', badge: '明细' },
              { title: '保存配方版本', description: '一次保存主档和有效明细行。', badge: '保存' },
              { title: '工单再执行', description: '领料、实耗、质检、成品入库去工单区完成。', badge: '执行' },
            ]}
            boundaries={[
      ...
      <div className="space-y-5">
        <div className="rounded-[32px] border border-white/50 bg-white/70 p-6 shadow-[0_20px_50px_rgba(0,0,0,0.03)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/70 space-y-5">
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-[20px] border border-blue-100 bg-blue-50/70 px-4 py-3 text-xs font-bold text-blue-800 dark:border-blue-900/40 dark:bg-blue-950/20 dark:text-blue-200">
            <span className="rounded-full bg-blue-600 px-3 py-1 text-white">当前对象：新配方草稿</span>
          ...
          <div className="rounded-[24px] border border-slate-100 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-800/50">
            ...
            >
              ...
              <span className="rounded-full bg-white px-3 py-1 text-xs font-black uppercase tracking-[0.18em] text-blue-600 shadow-sm dark:bg-slate-900">
        ...
        <div className="rounded-[32px] border border-white/50 bg-white/70 p-6 shadow-[0_20px_50px_rgba(0,0,0,0.03)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/70 space-y-5">
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-[20px] border border-emerald-100 bg-emerald-50/70 px-4 py-3 text-xs font-bold text-emerald-800 dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-200">
            <span className="rounded-full bg-emerald-600 px-3 py-1 text-white">当前对象：原料明细</span>
            <span>每一横行 = 一种原料；行号、字段和报错都只属于该行。</span>
          </div>
          {!isBomMasterReady ? (
          ...
          <div className="rounded-[28px] bg-slate-50 dark:bg-slate-800/70 border border-slate-100 dark:border-slate-700 p-5 space-y-4">
            ...
            {selectedBomProcessSummary ? (
              <div className="rounded-[22px] bg-white/80 dark:bg-slate-900/70 border border-slate-100 dark:border-slate-700 px-4 py-3">
                <div className="text-xs font-black uppercase tracking-[0.25em] text-slate-400 mb-2">工艺摘要</div>
                <div className="text-sm font-bold text-slate-700 dark:text-slate-200">{selectedBomProcessSummary}</div>
              </div>
            ) : null}
            {selectedBomQualitySummary ? (
              <div className="rounded-[22px] bg-white/80 dark:bg-slate-900/70 border border-slate-100 dark:border-slate-700 px-4 py-3">
                <div className="text-xs font-black uppercase tracking-[0.25em] text-slate-400 mb-2">质检规范</div>
                <div className="text-sm font-bold text-slate-700 dark:text-slate-200">{selectedBomQualitySummary}</div>
              </div>
            ) : null}
            <div className="space-y-2">
              <div className="text-xs font-black uppercase tracking-[0.25em] text-slate-400">配方明细快览</div>
              <div className="space-y-2">
