...
const SHOT_DIR = path.join(OUTPUT_DIR, 'focused-audit');
const REPORT_PATH = path.join(OUTPUT_DIR, 'focused-browser-audit-report.json');
...
async function testCrmDetailEdit(page) {
  ...
  await withTimebox(page, 'crm-open-first-customer', TIMEOUTS.readBack, async () => {
    ...
    targetName = (await firstRow.innerText()).split('\n').filter(Boolean).slice(0, 3).join(' / ');
    targetCustomerName = ((await firstRow.innerText()).split('\n').map((item) => item.trim()).find((item) => item && !item.startsWith('ID:') && !item.includes('待')) || '').trim();
    await firstRow.click();
    await page.waitForTimeout(800);
  });

  await withTimebox(page, 'crm-edit-note', TIMEOUTS.save, async () => {
    const notes = page.locator('textarea[placeholder*="备注"]').last();
  ...
  await withTimebox(page, 'crm-note-readback', TIMEOUTS.readBack, async () => {
    ...
    await firstRow.waitFor({ state: 'visible', timeout: TIMEOUTS.readBack });
    await firstRow.click();
    const notes = page.locator('textarea[placeholder*="主数据备注"], textarea[placeholder*="备注"]').last();
    await notes.waitFor({ state: 'visible', timeout: TIMEOUTS.modal });
    const started = Date.now();
  ...
  const shot = await safeScreenshot(page, 'crm-note-readback');
  recordStep({ step: 'crm-note-readback-evidence', result: 'passed', evidence: shot, targetName, note: DATA.customerNote });

  await withTimebox(page, 'crm-address-contact-edit', TIMEOUTS.save, async () => {
    const visibleInputs = page.locator('input:visible');
    await visibleInputs.nth(8).fill(DATA.addressLabel);
    await visibleInputs.nth(9).fill(DATA.addressRegisteredName);
    await visibleInputs.nth(10).fill(DATA.addressRegistrationNo);
    await visibleInputs.nth(11).fill(DATA.addressTaxNo);
    await visibleInputs.nth(12).fill(DATA.addressCountryCode);
    await page.locator('textarea:visible').nth(3).fill(DATA.addressFull);
    await visibleInputs.nth(13).fill(DATA.contactName);
    await visibleInputs.nth(14).fill(DATA.contactRole);
    await visibleInputs.nth(15).fill(DATA.contactPhone);
    await visibleInputs.nth(16).fill(DATA.contactEmail);
  ...
  await withTimebox(page, 'crm-address-contact-readback', TIMEOUTS.readBack, async () => {
    ...
    await firstRow.waitFor({ state: 'visible', timeout: TIMEOUTS.readBack });
    await firstRow.click();
    await waitForBodyText(page, DATA.contactEmail, TIMEOUTS.readBack);
  });

  const addressReadbackValue = await page.locator('textarea:visible').nth(3).inputValue().catch(() => '');
  ...
  return { customerName: targetCustomerName || targetName };
}
...
async function testProcurementCreate(page) {
  ...
  await withTimebox(page, 'procurement-create-supplier', TIMEOUTS.save, async () => {
    await page.locator('input[placeholder="供应商名称"]').fill(DATA.supplierName);
    await page.locator('input[placeholder="分类"]').fill(DATA.supplierCategory);
    await page.locator('input[placeholder="姓名"]').fill(DATA.supplierContact);
    await page.locator('input[placeholder="手机号"]').fill(DATA.supplierPhone);
    await page.locator('textarea').first().fill(`Alias ${RUN_ID}`);
    await page.locator('button.bg-blue-600').last().click();
    await page.waitForTimeout(1500);
    await page.locator('input[placeholder*="搜索"]').fill(DATA.supplierName);
  ...
  await withTimebox(page, 'procurement-create-order', TIMEOUTS.save, async () => {
    ...
    await page.locator('input[placeholder="产品"]').fill(DATA.orderItem);
  ...
  await withTimebox(page, 'procurement-approve-dispatch', TIMEOUTS.save, async () => {
    ...
    await page.locator('button[title="发运"]').first().click();
    await waitForBodyText(page, 'IN_TRANSIT', TIMEOUTS.readBack);
  });
...
async function testShippingOcrInteraction(page, customerName) {
  ...
  await withTimebox(page, 'shipping-clear-images', TIMEOUTS.readBack, async () => {
    ...
    await waitForBodyText(page, '拖拽图片或 PDF 到此处进行 AI 识别', TIMEOUTS.readBack);
  });

  const shot = await safeScreenshot(page, 'shipping-ocr-interaction');
  recordStep({ step: 'shipping-ocr-interaction-evidence', result: 'passed', evidence: shot, uploadCount: 1 });
  ...
  await withTimebox(page, 'shipping-ocr-parse-apply', TIMEOUTS.save, async () => {
    ...
    await page.locator('textarea').first().fill(ocrText);
