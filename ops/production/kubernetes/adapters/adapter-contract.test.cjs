...
const baseEnv = {
  ...
  HA_DRILL_CHANGE_TICKET: 'CHG-contract-1',
};
...
try {
  ...
  assert.throws(
    () => run(cnpgAdapter, 'fail-primary', { ...cnpgEnv, HA_DRILL_CHANGE_TICKET: '' }),
    /HA_DRILL_CHANGE_TICKET is required/,
  );
  ...
  assert.deepEqual(run(cnpgAdapter, 'old-primary-status', { ...cnpgEnv, FAKE_KEEP_POD_UID: 'true' }), { rejoinedAsReplica: false });
  assert.deepEqual(run(cnpgAdapter, 'old-primary-status', cnpgEnv), { rejoinedAsReplica: true });
  assert.throws(
    () => run(cnpgAdapter, 'old-primary-status', { ...cnpgEnv, CNPG_NAMESPACE: 'other-pilot' }),
    /state is missing/,
  );
  ...
  assert.throws(
    () => run(redisAdapter, 'fail-primary', { ...redisEnv, HA_DRILL_CHANGE_TICKET: '' }),
    /HA_DRILL_CHANGE_TICKET is required/,
  );
  ...
  assert.deepEqual(run(redisAdapter, 'old-primary-status', { ...redisEnv, FAKE_KEEP_POD_UID: 'true' }), { rejoinedAsReplica: false });
  assert.deepEqual(run(redisAdapter, 'old-primary-status', redisEnv), { rejoinedAsReplica: true });
  assert.throws(
    () => run(redisAdapter, 'old-primary-status', { ...redisEnv, REDIS_K8S_NAMESPACE: 'other-pilot' }),
    /state is missing/,
  );
  const objectEnv = {
