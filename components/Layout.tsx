...
const ModuleBadge: React.FC<{ id: string; active?: boolean; size?: 'sm' | 'md' | 'lg' }> = ({ id, active = false, size = 'md' }) => {
  ...
  return (
    <span className={`relative isolate inline-flex shrink-0 items-center justify-center overflow-hidden ${sizeClass} ${active
...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, onLogout }) => {
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-800 dark:text-slate-100 overflow-hidden font-sans selection:bg-blue-200 dark:selection:bg-blue-900">
      ...
      <aside className={`hidden lg:flex w-72 bg-white/60 dark:bg-slate-900/60 backdrop-blur-[24px] border-r border-white/40 dark:border-slate-800/50 flex-col z-30 transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none ${compactMode ? 'm-2 rounded-2xl' : 'm-5 rounded-[44px]'} shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] dark:shadow-none hover:shadow-blue-500/5`}>
        <div className="p-10">
          <div className="flex items-center space-x-4 mb-2 group">
            <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[22px] flex items-center justify-center text-white shadow-xl shadow-blue-500/30">
              <Command size={24} />
            </div>
            <div>
            ...
            <section key={section.group} className="space-y-2">
              ...
              {section.items.map((item) => (
                <button
                  ...
                  className={`group w-full flex items-center px-4 py-3 rounded-[24px] transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none   ${activeTab === item.id
                    ? 'bg-gradient-to-br from-blue-600 to-blue-700 text-white shadow-2xl shadow-blue-500/40 scale-100 z-10'
                    : 'text-slate-500 dark:text-slate-400 hover:bg-white/80 dark:hover:bg-slate-800/80  hover:shadow-lg hover:shadow-slate-200/40 dark:hover:shadow-none'
                    }`}
                >
        ...
        <div className="p-6 space-y-4">
          <div className="bg-slate-50/50 dark:bg-slate-800/30 rounded-[30px] p-4 flex items-center space-x-3 relative group border border-white/20">
            ...
            <button
              ...
              className="p-2 text-slate-300 hover:text-blue-500 transition-colors duration-150 motion-reduce:transition-none"
            ...
            {showPicker === 'role' && (
              <div className="absolute bottom-full left-0 mb-4 w-72 bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-white/40 dark:border-slate-800 p-4 z-[110] motion-safe:animate-in slide-in-from-bottom-4 zoom-in-95 duration-150">
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  <button onClick={() => { if (!setActiveTab('dashboard')) return; switchUser('manager', 'channel'); setShowPicker(null); }} className="group relative w-full text-left p-3 rounded-2xl text-sm font-black border border-slate-50 dark:border-slate-800 hover:border-blue-200 dark:hover:border-blue-800 hover:bg-blue-50/30 dark:hover:bg-blue-900/20 transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none flex items-center">
                    <div className="p-2 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 rounded-xl mr-3"><Network size={14} /></div>
                    <div>
                      <span>{t.roleManagerChannel}</span>
                      <p className="text-xs text-slate-400 font-bold">{t.distributionNetwork}</p>
                    </div>
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(['admin', 'sales', 'warehouse', 'finance'] as UserRole[]).map(r => (
                    <button
                      ...
                      className={`text-left p-3 rounded-2xl text-xs font-black transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none ${currentUser.role === r ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
                    >
          ...
          {onLogout && (
            <button
              onClick={onLogout}
              className="w-full flex items-center justify-center px-6 py-4 rounded-[28px] bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 hover:bg-red-500 hover:text-white transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none font-black text-xs uppercase tracking-wider gap-2 shadow-sm hover:shadow-red-200  group"
            >
              <LogOut size={16} />
              <span>{t.logout}</span>
            </button>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-[110] bg-slate-950/60 backdrop-blur-md" onClick={() => setMobileMenuOpen(false)}>
          <div className="absolute bottom-0 left-0 right-0 max-h-[90vh] bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-t-[48px] p-10 flex flex-col motion-safe:animate-in slide-in-from-bottom-[50%] duration-150 shadow-[0_-20px_60px_-15px_rgba(0,0,0,0.3)]" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between mb-10">
              ...
              <button onClick={() => setMobileMenuOpen(false)} className="p-4 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-500"><X size={24} /></button>
            </div>
            <button
              type="button"
              ...
              className="mb-5 flex min-h-12 w-full items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-left text-sm font-black text-slate-700  dark:border-slate-800 dark:bg-slate-800 dark:text-slate-100"
            >
            ...
            <div className="grid grid-cols-2 gap-4 overflow-y-auto pb-12 px-1">
              {menuItems.map((item) => (
                <button
                  ...
                  className={`group flex flex-col items-center justify-center p-8 rounded-2xl transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none  ${activeTab === item.id
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className={`${compactMode ? 'h-14 lg:h-16' : 'h-24 lg:h-28'} flex items-center justify-between px-6 lg:px-12 z-20 transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none`}>
          ...
          <div className="hidden 2xl:flex items-center space-x-3 2xl:space-x-4">
            <button
              ...
              className={`p-4 backdrop-blur-xl rounded-[22px] transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none shadow-sm border  group ${
                compactMode
            ...
            <button
              ...
              className="p-4 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-[22px] text-slate-500 hover:bg-amber-100 dark:hover:bg-indigo-900 hover:text-amber-600 dark:hover:text-indigo-300 transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none shadow-sm border border-white/40 dark:border-slate-800  group"
            >
              {theme === 'light' ? <Moon size={22} /> : <Sun size={22} />}
            </button>

            <button onClick={() => setShowAISettings(true)} aria-label={t.aiSettings} className="p-4 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-[22px] text-slate-500 hover:bg-blue-100 dark:hover:bg-blue-900/30 hover:text-blue-600 dark:hover:text-blue-400 transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none shadow-sm border border-white/40 dark:border-slate-800  group" title={t.aiSettings}>
              <Brain size={22} />
            </button>

            <div className="relative">
              <button onClick={() => setShowPicker(p => p === 'lang' ? null : 'lang')} className="flex items-center space-x-3 px-5 py-4 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-[22px] hover:shadow-xl transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none shadow-sm  border border-white/40 dark:border-slate-800 group">
                <div className="w-7 h-7 bg-sky-100/80 text-sky-600 rounded-full flex items-center justify-center shadow-inner">
                  <Languages size={16} strokeWidth={2.6} />
                </div>
                <span className="text-xs font-black tracking-tight uppercase">{language}</span>
              </button>
              {showPicker === 'lang' && (
                <div className="absolute right-0 top-full mt-4 w-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-[28px] shadow-2xl border border-white/40 dark:border-slate-800 p-2 z-[110] motion-safe:animate-in slide-in-from-top-4 duration-150">
                  {([
                    ['zh', '中文'],
                    ['en', 'English'],
                    ['vi', 'Tieng Viet'],
                    ...
                    <button
                      ...
                      className={`w-full text-left px-5 py-3.5 rounded-2xl text-sm font-black mb-1 transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none ${language === code ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                    >
                      {label}
                    </button>
                  ))}
            ...
            <div className="relative">
              <button onClick={() => setShowPicker(p => p === 'curr' ? null : 'curr')} className="flex items-center space-x-3 px-5 py-4 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-[22px] hover:shadow-xl transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none shadow-sm  border border-white/40 dark:border-slate-800 group">
                <div className="w-7 h-7 bg-emerald-100/80 text-emerald-600 rounded-full flex items-center justify-center shadow-inner">
              ...
              {showPicker === 'curr' && (
                <div className="absolute right-0 top-full mt-4 w-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-[28px] shadow-2xl border border-white/40 dark:border-slate-800 p-2 z-[110] motion-safe:animate-in slide-in-from-top-4 duration-150">
                  {['CNY', 'USD', 'VND'].map(c => (
                    <button key={c} onClick={() => { setCurrency(c as Currency); setShowPicker(null); }} className={`w-full text-left px-5 py-3.5 rounded-2xl text-sm font-black mb-1 transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none ${currency === c ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-slate-100 dark:hover:bg-slate-800'}`}>{c}</button>
                  ))}
                </div>
              )}
            </div>
        ...
        <div
          ...
          className="flex-1 overflow-y-auto px-6 pt-4 pb-32 lg:px-12 lg:py-8 no-scrollbar transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-150 motion-reduce:transition-none"
        >
        ...
        <div className="lg:hidden fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-4 right-4 h-[88px] bg-white/86 dark:bg-slate-950/86 backdrop-blur-2xl rounded-2xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.25)] border border-white/50 dark:border-slate-800/50 flex items-center justify-evenly px-3 z-[90] motion-safe:animate-in slide-in-from-bottom-12 duration-150">
          ...
          <button
            ...
            className="flex h-14 min-w-14 flex-col items-center justify-center rounded-[18px] bg-blue-50 text-blue-700  dark:bg-blue-950/70 dark:text-blue-200"
            onClick={openMobileAIAssistant}
          >
            <Bot size={24} />
            <span className="mt-1 text-xs font-black">AI</span>
          </button>
          <button data-testid="mobile-menu-open" aria-label={t.navQuickJump || '打开导航菜单'} title={t.navQuickJump || '打开导航菜单'} className="flex h-14 min-w-14 flex-col items-center justify-center rounded-[18px] text-slate-600 bg-slate-100/90 dark:bg-slate-800/90 dark:text-slate-200" onClick={() => setMobileMenuOpen(true)}>
            <Menu size={26} />
          </button>
        </div>
