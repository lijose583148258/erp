...
const variantStyles: Record<NavigatorVariant, {
  ...
  amber: {
    ...
    inactive: 'border-slate-100 bg-white/80 text-slate-600 hover:border-amber-200 hover:text-slate-950 dark:border-slate-800 dark:bg-slate-900/80 dark:text-slate-300',
...
export function WorkspaceTaskNavigator<T extends string>({
  ...
  const titleId = `workspace-task-title-${generatedId}`;
  const descriptionId = `workspace-task-description-${generatedId}`;
  ...
  return (
    <section
      data-workspace-task-navigator
      aria-labelledby={titleId}
      aria-describedby={descriptionId}
      className={`${compact ? 'rounded-[28px] p-3' : 'rounded-[40px] p-4'} border border-white/60 bg-white/70 shadow-[0_18px_50px_rgba(15,23,42,0.05)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/55 ${className}`}
    >
      <div className={`flex flex-col ${compact ? 'gap-3' : 'gap-4'} ${columns === 'three' ? 'xl:flex-row xl:items-center xl:justify-between' : ''}`}>
        <div className={compact ? 'px-1' : 'px-2'}>
          ...
          <h2 id={titleId} className={`${compact ? 'mt-1 text-lg' : 'mt-2 text-2xl'} font-black tracking-tight text-slate-950 dark:text-white`}>{title}</h2>
          <p id={descriptionId} className={`${compact ? 'sr-only' : 'mt-2 max-w-3xl text-sm font-bold leading-6 text-slate-500 dark:text-slate-400'}`}>{description}</p>
        </div>
        <div role="tablist" aria-label={title} className={`grid gap-3 ${columnStyles[columns]}`}>
          {items.map((item, index) => {
            ...
            return (
              <button
                ...
                className={`${compact ? 'rounded-[20px] p-3' : 'rounded-[28px] p-4'} min-h-11 border text-left transition-[background-color,border-color,color,box-shadow] duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 motion-reduce:transition-none ${active ? styles.active : styles.inactive}`}
              >
