...
|---|---|---|---|
| Production database | `backend/prisma/schema.prisma` uses SQLite and both Dockerfiles default `DATABASE_URL=file:/data/stable.db`; runtime already blocks SaaS mode on SQLite and blocks PostgreSQL URL with the SQLite Prisma artifact | Record as out of scope, do not migrate schema here | PostgreSQL artifact/schema split, migration rehearsal, rollback, and deployment config |
| TypeScript strictness | root `tsconfig.json` has no `strict`; `backend/tsconfig.json` already has `strict: true` | Do not flip frontend strict in PR2 | frontend strict rollout with typed service boundaries and focused fixes |
...
| Shared contracts | root `types.ts` manually mirrors API/domain shapes while Prisma models live under `backend/prisma/models/*.prisma` | Treat as contract drift risk only | generated/shared API DTOs and OpenAPI schema PR |
| Client/server state | app shell uses React Context and service calls; there is no TanStack Query/Zustand/SWR dependency | Keep as product readiness lead | introduce server-state cache on one high-value list before broad adoption |
| Route loading and errors | `app/appContent.tsx` already uses `React.lazy` and `PageErrorBoundary` for active pages | Correct old claim; not missing | verify coverage and add regression tests if gaps appear |
| Large tables | no `@tanstack/react-virtual` or equivalent virtual list dependency; grids rely on pagination/search and overflow | Keep as commercial table scoring risk | virtualized/server-side grid path after Smart Filter contract |
| PWA/offline | `/sw.js` deliberately returns 404 and offline policy is documented as a security question | Do not add service worker in PR2 | stale-data policy, cache boundaries, and mobile offline read/write rules |
| Observability | backend exposes `/metrics`, `/health`, `/ready`; no front-end RUM/Web Vitals ingestion loop in active app code | Do not call this production-complete | browser RUM/Web Vitals collection, backend aggregation, alerting |
| API versioning | active routes are mounted under `/api/...`; no `/api/v1` route namespace was found in backend route registration | Out of PR2 | versioned API namespace and compatibility policy |
| Realtime collaboration | no `socket.io`, `WebSocket`, `EventSource`, or SSE evidence found in active app/backend search | Out of PR2 | scoped realtime notification design after event/audit model is stable |
| File storage | backend serves local `getUploadDir()` through `/uploads`; contract and POD files build local upload URLs; no S3/MinIO/storage adapter evidence found | Out of PR2 | file storage abstraction, object-store adapter, retention and access policy |
| Search engine | customer/order/procurement/warehouse searches rely on Prisma `contains` or SQL `LIKE`; no dedicated search engine path found | Out of PR2 | search strategy for SKU/customer/order text, indexing, permissions, and relevance |
| Unified cache | `backend/package.json` includes `redis` and `ioredis`, but active source evidence shows only local/in-memory or SQLite cache behavior, such as currency cache and PRAGMA cache | Out of PR2 | Redis/cache ownership, invalidation rules, metrics, and fallback policy |
...
| API documentation | no active OpenAPI/Swagger runtime route or generated contract found in source search; SDK/webhook evidence was not found | Out of PR2 | OpenAPI-first API contract, generated DTO/SDK boundary, webhook roadmap |
| Security collaboration | helmet CSP exists; JWT secret production guard exists; no CSRF/MFA/secret-manager evidence found in active source search | Out of PR2 | CSRF/session model review, MFA, secret manager, security ADR |
| Engineering docs | root-level `CONTRIBUTING*`, `CHANGELOG*`, and docs ADR files were not found in the current search; a backend service string references a retained ADR but no active root governance set is present | Out of PR2 | ADR, CONTRIBUTING, CHANGELOG, release notes and ownership docs |
| Frontend component tests | backend Jest config exists and `utils/customerName.test.ts` exists, but no broad frontend Vitest/RTL component test foundation was found in root package scripts or dependencies | Out of PR2 | frontend component/unit test foundation for grids, filters, dashboard states, and AI surfaces |

Recommended follow-up order after PR2:

1. PostgreSQL production artifact and migration rehearsal.
2. Shared API contract: OpenAPI + generated DTO/SDK boundary.
3. Frontend server-state and Smart Filter contract on one revenue/receivable route.
...
6. File storage abstraction, search/cache strategy, JSON normalization, realtime notifications, and MFA/security hardening.
