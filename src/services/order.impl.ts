...
const mapOrderResponse = (value: unknown): SalesOrder => {
    ...
    const order: SalesOrder = {
        ...
        receivableAdjustmentAmount: item.receivableAdjustmentAmount != null ? toNumberValue(item.receivableAdjustmentAmount) : undefined,
        effectiveReceivableAmount: item.effectiveReceivableAmount != null ? toNumberValue(item.effectiveReceivableAmount) : undefined,
        outstandingAmount: item.outstandingAmount != null ? toNumberValue(item.outstandingAmount) : undefined,
...
export type OrderListParams = {
    ...
    startDate?: string;
    endDate?: string;
    sortBy?: 'createdAt' | 'updatedAt' | 'orderNo' | 'finalAmount' | 'paidAmount' | 'status' | 'paymentStatus';
    sortOrder?: 'asc' | 'desc';
};
...
export const orderService = {
    async getPage(params: OrderListParams = {}, options: ApiRequestOptions = {}): Promise<OrderPage> {
        ...
        const response = await api.get<unknown, ApiDataResponse<unknown[]>>(`/orders?${query.toString()}`, { signal: options.signal });
        const rows = Array.isArray(response.data) ? response.data.map(mapOrderResponse) : [];
        return {
            rows,
